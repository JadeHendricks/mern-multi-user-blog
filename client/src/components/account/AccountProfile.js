...
const Profile = ({ match }) => {
    ...
    const { getAllUsersPosts, getUser, user, userPosts, loading } = useContext(UserContext);
                            ...
                            <aside className="account__options">
                                <nav className="account-nav">
                                    <ul className="account-nav__ul">
                                        ...
                                        { loggedInUser && isUsersData(loggedInUser._id, user._id) && <li className="account-nav__el"><a onClick={ handleNavigationState } name='account-settings' href="#!">Account settings</a></li> }
                                    </ul>
                                </nav>
                            </aside>
                                ...
                                { navigationState === 'all-posts' ? (
                                    <section className="account-posts">
                                        <div className="cards cards--account">
                                            { loggedInUser && isUsersData(loggedInUser._id, user._id) && userPosts?.map(post => <ProfileCard key={post._id} post={ post } />) }
                                            { loggedInUser && !isUsersData(loggedInUser._id, user._id) && userPosts?.map(post => <PostCard key={post._id} post={ post } />) }
                                        </div>
                                    </section>
